...
function ISMPButton:initialise()
	ISPanel.initialise(self);
end

function ISMPButton:onMouseMove(dx, dy)
	self.mouseOver = self:isMouseOver();
end

function ISMPButton:onMouseMoveOutside(dx, dy)
...
function ISMPButton:setJoypadFocused(focused)
...
function ISMPButton:onMouseDown(x, y)
	if not self:getIsVisible() then
		return;
	end
    self.pressed = true;
...
function ISMPButton:onMouseDoubleClick(x, y)
	return self:onMouseDown(x, y)
end

function ISMPButton:forceClick()
...
function ISMPButton:setJoypadButton(texture)
    self.isJoypad = true;
    self.joypadTexture = texture;
end

function ISMPButton:clearJoypadButton()
    self.isJoypad = false;
    self.joypadTexture = nil;
end

function ISMPButton:prerender()
...
function ISMPButton:setImage(image)
	self.image = image;
end

function ISMPButton:forceImageSize(width, height)
    self.forcedWidthImage = width;
    self.forcedHeightImage = height;
end

function ISMPButton:setOverlayText(text)
...
--        print("btn:image")
		...
		if self.forcedWidthImage and self.forcedHeightImage then
            self:drawTextureScaledAspect(self.image, 5, (self.height / 2) - (self.forcedHeightImage / 2),self.forcedWidthImage,self.forcedHeightImage, alpha, self.textureColor.r, self.textureColor.g, self.textureColor.b);
        elseif self.image:getWidthOrig() <= self.width and self.image:getHeightOrig() <= self.height then
            self:drawTexture(self.image, 5, (self.height / 2) - (self.image:getHeightOrig() / 2), alpha, self.textureColor.r, self.textureColor.g, self.textureColor.b);
        else
            self:drawTextureScaledAspect(self.image, 5, 0, self.width, self.height, alpha, self.textureColor.r, self.textureColor.g, self.textureColor.b);
        end
	end
	local textW = 0;
	local height = 0;
	...
	local x = 5 + self.forcedWidthImage + 5
...
function ISMPButton:setFont(font)
	self.font = font;
end

function ISMPButton:getTitle()
	return self.title;
end

function ISMPButton:setTitle(title)
	self.title = title;
end

function ISMPButton:setOnMouseOverFunction(onmouseover)
	self.onmouseover = onmouseover;
end

function ISMPButton:setOnMouseOutFunction(onmouseout)
	self.onmouseoutfunction = onmouseout;
end

function ISMPButton:setDisplayBackground(background)
	self.displayBackground = background;
end

function ISMPButton:update()
...
function ISMPButton:updateTooltip()
...
function ISMPButton:setRepeatWhilePressed(func)
	self.repeatWhilePressedFunc = func
end

function ISMPButton:setBackgroundRGBA(r, g, b, a)
...
function ISMPButton:setBackgroundColorMouseOverRGBA(r, g, b, a)
...
function ISMPButton:setBorderRGBA(r, g, b, a)
...
function ISMPButton:setTextureRGBA(r, g, b, a)
...
function ISMPButton:enableAcceptColor()
...
function ISMPButton:enableCancelColor()
...
function ISMPButton:toggleAcceptCancel(bEnabled)
	if bEnabled then
...
function ISMPButton:setEnable(bEnabled)
...
function ISMPButton:isEnabled()
	return self.enable;
end

function ISMPButton:setTooltip(tooltip)
    self.tooltip = tooltip;
end

function ISMPButton:setWidthToTitle(minWidth, isJoypad)
	local width = getTextManager():MeasureStringX(self.font, self.title) + 5 * 2
	if isJoypad or self.iconTexture then
		width = width + 5 + self.joypadTextureWH
	end
    if self.image and self.forcedWidthImage then
        width = width + 5 + self.forcedWidthImage
    end
...
function ISMPButton:setOnClick(func, arg1, arg2, arg3, arg4)
	self.onclick = func
	self.onClickArgs = { arg1, arg2, arg3, arg3 }
end

function ISMPButton:setSound(which, soundName)
	self.sounds[which] = soundName
end

function ISMPButton:calculateLayout(_preferredWidth, _preferredHeight)
