...
function ISPrintMediaTextPanel:setText(text)
	self.text = text or ""
end
...
--]]
    ...
    if string.find(command, "MAP:") ~= nil then
        local args = string.sub(command, 5)
        local vs = string.split(args, ",")
        local mapID = string.trim(vs[1])
        local w = tonumber(string.trim(vs[2]))
        local h = tonumber(string.trim(vs[3]))
        local mx = (self.width - self.marginLeft - self.marginRight) / 2
        local map = {
            x = mx - w / 2,
            y = y,
...
function ISPrintMediaTextPanel:replaceKeyName(text, offset)
...
function ISPrintMediaTextPanel:replaceKeyNames(text)
	local offset = 1
	while true do
...
function ISPrintMediaTextPanel:onMouseWheel(del)
...
function ISPrintMediaTextPanel:paginate()
	...
	self.maps = {}
	self.rgb = {};
	self.rgbCurrent = { r = 0.0, g = 0.0, b = 0.0 }
	self.rgbStack = {}
	self.orient = {}
	self.indent = 0
...
function ISPrintMediaTextPanel:setContentTransparency(alpha)
...
function ISPrintMediaMap:new(x, y, width, height)
...
function ISPrintMediaTextPanel:renderMap(index, map)
	...
	if not self.mapUI[index] then
		local height = (map.h > 0) and map.h or 100
		local width = (map.w > 0) and map.w or 100
		self.mapUI[index] = ISPrintMediaMap:new(self.marginLeft + map.x, self.marginTop + map.y, width, height)
		self:addChild(self.mapUI[index])
		self:setScrollChildren(true)
		self.mapUI[index]:setScrollWithParent(true)
	end
	local mapUI = self.mapUI[index]
	if mapUI.mapID ~= map.mapID then
		mapUI.mapID = map.mapID
		PrintMediaMaps.callLua("Init", mapUI, index)
		if map.w == 0 then
			mapUI.javaObject:scaleWidthToHeight()
			local mx = (self.width - self.marginLeft - self.marginRight) / 2
			mapUI:setX(self.marginLeft + mx - mapUI.width / 2)
		end
		mapUI.mapAPI:setZoom(mapUI.mapAPI:getBaseZoom())
	end
end
...
function ISPrintMediaTextPanel:setMargins(left, top, right, bottom)
