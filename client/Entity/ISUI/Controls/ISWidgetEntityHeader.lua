...
function ISWidgetEntityHeader:initialise()
	ISPanel.initialise(self);
end

function ISWidgetEntityHeader:createChildren()
    ...
    local titleStr = "Unknown Object";
    if self.entityStyle then
        titleStr = self.entityStyle:getDisplayName();
    elseif self.entity then
        titleStr = self.entity and self.entity:getEntityDisplayName() or titleStr;
    end
    ...
    if self.enableInfoButton and self.entityStyle and self.entityStyle:getDescription() then
        ...
        self.buttonInfo.onclick = ISWidgetEntityHeader.onButtonClick;
...
function ISWidgetEntityHeader:calculateLayout(_preferredWidth, _preferredHeight)
...
function ISWidgetEntityHeader:render()
    ISPanel.render(self);
end

function ISWidgetEntityHeader:update()
    ISPanel.update(self);
end

function ISWidgetEntityHeader:new(x, y, width, height, player, entity, entityStyle, _styleIcon, _styleLabel, _styleButton)
	local o = ISPanel:new(x, y, width, height);
    setmetatable(o, self)
    self.__index = self
    o.player = player;
    o.entity = entity;
    o.entityStyle = entityStyle;
